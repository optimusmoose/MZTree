...
class RectangularTraceCommand extends Command { 
     ...
     * @param dataStorage StorageFacade implementation from which to perform a rectangular point query
     ...
     */
    public RectangularTraceCommand(MzTree mzTree, StorageFacade dataStorage, short traceID, Double[] bounds, boolean isAdd) {
        ...
        try{
            ...
            List<MsDataPoint> area = mzTree.query(bounds[0], bounds[1], bounds[2].floatValue(), bounds[3].floatValue(), Integer.MAX_VALUE, Integer.MIN_VALUE, false);
            ...
            if(isAdd && !mzTree.traceMap.containsKey(toTraceID))
            {
                ...
                this.doCommand = () -> { 
                    mzTree.insertTrace(toTraceID, (short)0);
        ...
        {
            System.err.println("Could not perform rectangular query to data storage || " + e.getMessage());
            e.printStackTrace();
        }   
    }
} 
...
class TraceCommand extends Command {
     ...
     */
    public TraceCommand(MzTree mzTree, short newTraceID, Integer[] pointIDs) 
    {
        // resolve the oldTraceID for the undo command
        // due to the yellow rule this can only be 0, or the same traceID, for all points
        short oldTraceID = mzTree.pointCache.get(pointIDs[0]).traceID;
        ...
        else
        {
            ...
            this.doCommand = () -> {
                // create trace entity
                mzTree.insertTrace(newTraceID, (short)0);
...
class EnvelopeCommand extends Command {
     ...
     */
    public EnvelopeCommand(MzTree mzTree, short newEnvelopeID, Integer[] traceIDs) 
    {
        ...
        short oldEnvelopeID = mzTree.traceMap.getOrDefault(traceIDs[0].shortValue(), (short)0);
